...
class TestHtfCounterReaction:
    ...
    @pytest.mark.parametrize(
        ("htf_bias", "ltf_bias", "direction", "ichoch_break", "pro_isb_break"),
        [
            ("bearish", "bullish", "long", "up", "down"),
            ("bullish", "bearish", "short", "down", "up"),
        ],
    )
    def test_ltf_counter_ichoch_isb_sequence_rejects_pro_direction_isb(
        self,
        htf_bias,
        ltf_bias,
        direction,
            ...
            _fused(
                ...
                ltf_internal_structure_sequence=[
                    ...
                    },
                    {
                        ...
                        "eventAction": "structure_isb",
                        "eventCode": "SC05",
                        "breakDirection": pro_isb_break,
                        "eventTimestamp": "2025-01-01T10:15:00",
                        "levelTimestamp": "2025-01-01T10:00:00",
                        "levelSide": "low" if direction == "long" else "high",
                        "levelPrice": 1.1312,
                    },
                ],
            ),
        ...
        assert choch.debug_facts["ltf_counter_sequence_ichoch_direction"] == ichoch_break
        assert choch.debug_facts["ltf_counter_sequence_isb_direction"] is None
    ...
    def test_ltf_counter_ichoch_isb_sequence_accepts_intervening_pro_ichoch(self):
        ec = EvidenceCompiler()
        candidates = ec.update(
            _fused(
                htf_bias="bearish",
        ...
        assert choch.debug_facts["ltf_counter_ichoch_isb_sequence_seen"] is True
        assert choch.debug_facts["ltf_counter_sequence_invalidated"] is False
        assert choch.debug_facts["ltf_counter_sequence_invalid_reason"] is None
        assert choch.debug_facts["ltf_counter_ichoch_seen"] is True
        assert choch.debug_facts["ltf_counter_ichoch_direction"] == "up"
        assert choch.debug_facts["ltf_counter_ichoch_event_at"] == "2025-01-01T10:00:00"
        assert choch.debug_facts["ltf_counter_ichoch_source_store"] == "internal_structure_sequence"
        assert choch.debug_facts["ltf_pro_ichoch_seen"] is True
        assert choch.debug_facts["ltf_pro_ichoch_direction"] == "down"
        assert choch.debug_facts["ltf_pro_ichoch_event_at"] == "2025-01-01T10:10:00"
        assert choch.debug_facts["ltf_pro_ichoch_source_store"] == "internal_structure_sequence"
        assert "ltf_counter_isb_seen" not in choch.debug_facts
        assert "ltf_counter_isb_event_at" not in choch.debug_facts

    def test_ltf_counter_ichoch_isb_sequence_accepts_intervening_pro_ichoch_short_mirror(self):
        ec = EvidenceCompiler()
        candidates = ec.update(
            _fused(
                htf_bias="bullish",
        ...
        assert choch is not None
        assert choch.debug_facts["ltf_counter_ichoch_isb_sequence_seen"] is True
        assert choch.debug_facts["ltf_counter_sequence_trade_direction"] == "short"
        assert choch.debug_facts["ltf_counter_sequence_invalidated"] is False
        assert choch.debug_facts["ltf_counter_sequence_invalid_reason"] is None
        assert choch.debug_facts["ltf_counter_ichoch_seen"] is True
        assert choch.debug_facts["ltf_counter_ichoch_direction"] == "down"
        assert choch.debug_facts["ltf_counter_ichoch_event_at"] == "2025-01-01T10:00:00"
        assert choch.debug_facts["ltf_counter_ichoch_source_store"] == "internal_structure_sequence"
        assert choch.debug_facts["ltf_pro_ichoch_seen"] is True
        assert choch.debug_facts["ltf_pro_ichoch_direction"] == "up"
        assert choch.debug_facts["ltf_pro_ichoch_event_at"] == "2025-01-01T10:10:00"
        assert choch.debug_facts["ltf_pro_ichoch_source_store"] == "internal_structure_sequence"
        assert "ltf_counter_isb_seen" not in choch.debug_facts
        assert "ltf_counter_isb_event_at" not in choch.debug_facts
    ...
    def test_ltf_counter_internal_pressure_ignores_final_pro_ichoch(self):
        ec = EvidenceCompiler()
        candidates = ec.update(
            _fused(
                htf_bias="bullish",
        ...
        assert choch is not None
        assert choch.debug_facts["ltf_counter_internal_pressure_seen"] is True
        assert choch.debug_facts["ltf_counter_internal_pressure_class"] == "single_isb"
        assert choch.debug_facts["ltf_counter_internal_pressure_invalidated"] is False
        assert choch.debug_facts["ltf_counter_internal_pressure_invalid_reason"] is None

    def test_ltf_counter_internal_pressure_rejects_pro_isb_contradiction(self):
        ec = EvidenceCompiler()
        candidates = ec.update(
            _fused(
                htf_bias="bullish",
        ...
        assert choch.debug_facts["ltf_counter_internal_pressure_invalidated"] is True
